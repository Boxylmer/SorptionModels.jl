...
function MolarVolumeAnalysis(sorptionmodel::SorptionModel, pressures_mpa::AbstractVector{<:Number}, frac_dilations::AbstractVector{<:Number}, 
    isothermal_compressability=0; uncertainty_method=nothing, n_interp=30, modeltype=EmpiricalDilation())
    
    concentrations = [predict_concentration(sorptionmodel, p) for p in pressures_mpa]
    ...
    # conc(p) = predict_concentration(sorptionmodel, p)
    # @show [conc(p) for p in pressures_mpa]
    # @show dc_dp = ForwardDiff.derivative.(conc, pressures_mpa) # mpa / cc/cc
    # dp_dc_values = 1 ./ ForwardDiff.derivative.(conc, pressures_mpa) # mpa / cc/cc
    dp_dc_values = dp_dc.(sorptionmodel, pressures_mpa)

    if typeof(modeltype) <: EmpiricalDilation
        dilation_model = fit_model(modeltype, pressures_mpa, frac_dilations; uncertainty_method)
    elseif typeof(modeltype) <: DualModeDilation
    ...
    continuous_dilation_derivatives = predict_dilation_derivative(dilation_model, pressures_mpa)
    ...
    diagnostic_dp_dc = dp_dc.(sorptionmodel, diagnostic_pressures_mpa)
    ...
    volumes = (continuous_dilation_derivatives .+ isothermal_compressability) .* dp_dc_values .* MembraneBase.CC_PER_MOL_STP # (1/MPa) * (Mpa / (cc/cc)) * (cc/mol) = cm3/mol
    return MolarVolumeAnalysis(pressures_mpa, concentrations, dp_dc_values, frac_dilations, continuous_dilations, continuous_dilation_derivatives, volumes,
